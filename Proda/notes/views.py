...
def delete_note(request, dir_id, note_id):
    Note.objects.get(pk=note_id).delete()
    dir = NoteDirectory.objects.get(pk=dir_id)
    note_id = Note.objects.filter(owner=request.user.pk).filter(directory=dir).last().pk
    return HttpResponseRedirect(reverse('notes_dir_w_note', args=[dir_id, note_id]))


def add_note(request, dir_id):
    dir = NoteDirectory.objects.get(pk=dir_id)
    note = Note.objects.create(title='', content='', owner=request.user, directory=dir)
    return HttpResponseRedirect(reverse('notes_dir_w_note', args=[dir_id ,note.pk]))
...
def add_dirrectory(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Note.objects.create(title='', owner=request.user, directory=dir)
...
class NoteDirectoriesView(DataMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        if 'note_id' in self.kwargs:
            c_def = self.get_user_context(dir_id=self.kwargs['dir_id'],note_id=self.kwargs['note_id'])
        else:
